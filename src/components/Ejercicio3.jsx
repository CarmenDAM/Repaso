...
class Ejercicio3 extends React.Component {
  ...
  async componentDidMount(){
    const response = await fetch("https://www.etnassoft.com/api/v1/get/?category=programacion&lang=spanish&num_items=5");
    const data = await response.json();
    this.setState({listaLibros:data});
  }
...
<div>
  <h1>LISTA DE LIBROS</h1>
        <ul>
      {this.state.listaLibros.map(
        function GeneraElemento(item){
        return<li>Titulo: {item.title}</li>;
      })}
      </ul>
